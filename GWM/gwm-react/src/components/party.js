          ...
          <Stack alignItems="center" spacing={1}>
            ...
            <Stack direction="row" spacing={1}>
              ...
              <Chip
                ...
                label={user.isCreator ? "Party Creator" : "Member"}
              />
            </Stack>
          </Stack>
        </CardContent>
    ...
    if (games.length === 0) {
      fetch(`http://localhost:8080/Gwm-war/webresources/party/games`)
        ...
        .then((data) => {
          ...
          setGameId(
            availableGames.length === 0 ? null : availableGames[0].gameId
          );
        });
    }
    ...
    <Card sx={{ maxWidth: "800", mb: 2 }} raised>
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  You will {requestSign >= 0 ? "earn $ " : "offer $ "}
                </InputAdornment>
              ),
            }}
          />
      ...
      <CardContent>
        <Stack direction="row" justifyContent="space-between">
          ...
          {party.mode === "current" && party.partyOwner.userId === uId && (
            <Stack spacing={2} direction="row">
              ...
              <Button variant="contained" color="warning" onClick={endParty}>
                End party
              </Button>
              <Button variant="contained" color="error" onClick={deleteParty}>
                Delete party
              </Button>
            </Stack>
          )}
          ...
          <Grid container spacing={2}>
            {party.users.map((user) => (
              <UserCard
                key={user.userId}
                {...user}
                isCreator={party.partyOwner.userId === user.userId}
              />
            ))}
          </Grid>
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={8}>
              <List>
                {party.post === undefined || party.post.request.length === 0 ? (
                  ...
                  party.post.request.map((request) => {
                    return <Request key={request.requestId} {...request} />;
                  })
                )}
              </List>
            </Grid>
              ...
              <Grid item xs={12}>
                <Stack spacing={1}>
                  <Button
                    sx={{ width: "50%" }}
                    onClick={handleOpen}
                    color={party.post === undefined ? "success" : "warning"}
                    variant="contained"
                  >
                    {party.post === undefined ? "Create a " : "Edit your "} Post
                  </Button>
                ...
                return (
                  <Grid item xs={12} sm={6} md={4}>
                    <Card sx={{ p: 2, m: 2 }} raised>
                      <Typography variant="body1">
                        Review by <UserView uId={review.userId} />
                      </Typography>
      ...
      <Grid container spacing={3} sx={{ m: 0, width: "100%" }}>
        ...
        <Grid item xs={8} md={9}>
          ...
          >
            ...
            {parties.length === 0 ? (
              <Typography sx={{ mt: 2 }}>You have no {mode} parties</Typography>
            ) : (
              parties.map((party) => (
                <Party
                  key={party.partyId}
